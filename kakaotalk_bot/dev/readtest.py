...
with open(r"kakaotalk_bot\dev\chatlog.txt", "r", encoding="UTF-8") as f:
    lines = f.readlines()
...
def chat_check_command(cls, clst):
    ...
    if getText.iloc[-2, 0] != clst:
        ...
        if "조용" in getStr:
            kakao_sendtext("죄송합니다. 5분간 반응하지 않습니다.")
            time.sleep(300)
            return

        if "개발자" in getStr:
            kakao_sendtext("개발자는 현재 자리에 없습니다.")
            return
        ...
        for line in lines:
            ...
            if check > 5:
                ...
                break
            if getStr.find(line) != -1 or SequenceMatcher(None, getStr, line).ratio() > 0.5:
                print(getStr+" "+line+" "+str(SequenceMatcher(None, getStr, line).ratio()))
                check += 1
